...
const productos = [{id:"RaquetaWilson",title:"Raqueta Wilson",price:'90.000',description:"Raqueta Wilson Blade 98 V7",img:"raquetaWilson",categoryId:1},
...
const Categorias = () => {
    ...
    useEffect(() => {
        setIsLoading(true);
        const selectedProduct = productos.find((producto)=> producto.categoryId === id)
        if (selectedProduct){
            setdatos(selectedProduct)
            setIsLoading(false)
        }
    }, [id]);
    ...
    return <Navbar key={datos.categoryId} datos={datos}/> 
}
